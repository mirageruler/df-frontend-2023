...
export default function Home() {
  ...
  useEffect(() => {
    msgs.current.push({"role": "system", "content": `You are playing a game of chess against a user. Please tell me your move in algebraic notation (eg: e2-e4, eg: Be7, eg: Nxc4) and nothing else. Do not mention my moves.`});
  }, []);
  ...
  async function makeBOTMove() {
    if (game.isGameOver() || game.isDraw() || botPossibleMoves.current.length === 0) return;

    const botPossibleMovesString = botPossibleMoves.current.join(" or ");
    msgs.current.push({"role": "user", "content": `I play ${recentMove.current}. Your turn with the black side; and you must choose only one best move from the following available moves: ${botPossibleMovesString}.`});
  ...
  function onDrop(source: string, target: string, piece: string) {
    ...
    safeGameMutate((game) => {
      try {
        move = game.move({ from: source, to: target, promotion: piece[1].toLowerCase() ?? "q", });
      } catch (error) {
        return 'snapback';
      }
    });
